...
class TencentSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        list_url = []
        offset = 0
        for offset_temp in range(start_page, total + 1, page_size):
            offset = offset_temp
            list_url.append(self.domain + '/x/bu/pagesheet/list?_all=1&append=1&channel='+movie_type+'&listpage=2&offset='+(str)(offset_temp)+'&pagesize='+(str)(page_size)+'&sort=18')
        if (offset != total):
            list_url.append(self.domain + '/x/bu/pagesheet/list?_all=1&append=1&channel='+movie_type+'&listpage=2&offset='+(str)(offset)+'&pagesize='+(str)(page_size)+'&sort=18')
        ...
        if (movie_type == 'movie'):
            for list_url_item in list_url:
                ...
                for each in html.xpath("/html/body/div[@class='list_item']"):
                    count += 1
                    ...
                    movie_item['type'] = '电影'
                    movie_item['type2'] = get_str_from_xpath(each.xpath('./div[5]/div/a/text()')) + '片'
                    ...
                    if (movie_item['release_date'] == '会员看全集'): movie_item['release_date']
            ...
            for list_url_item in list_url:
                ...
                for each in html.xpath("/html/body/div[@class='list_item']"):
                    ...
                    if (nick_name_text == '别　名:'):
                        ...
                        movie_item['actors'] = actors
                        movie_item['type'] = '电视剧'
                        movie_item['type2'] = reverse_type2(get_str_from_xpath(each.xpath('./div[5]/div/a/text()')) + '剧')
                        ...
                        movie_item['release_date'] = get_str_from_xpath(each.xpath('./div[4]/div/span[2]/text()'))
                        movie_item['duration'] = '0'
                        movie_item['update_time'] = get_current_time()
                        movie_item['description'] = get_str_from_xpath(each.xpath('./div[6]/span[2]/span/text()'))
        ...
        if (movie_type == 'variety'):
            for list_url_item in list_url:
                ...
                for each in html.xpath("/html/body/div[@class='list_item']"):
                    ...
                    html = get_one_page(self.domain + '/detail/n/' + movie_id + '.html')
                    html = etree.HTML(html)
                    movie_item['score'] = get_str_from_xpath(html.xpath('/html/body/div[2]/div[1]/div/div/div/div[1]/div/span[1]/text()'))
                    html = get_one_page(self.domain + '/x/cover/' + movie_id + '.html')
                    html = etree.HTML(html)
                    # 视频已爬取且未更新
                    dic = {'name': movie_item['name']}
                    ...
                    movie_item['region'] = reverse_region(get_str_from_xpath(html.xpath('//*[@id="container_player"]/div/div[2]/div[1]/div[2]/div/div[3]/a[1]/text()')))
